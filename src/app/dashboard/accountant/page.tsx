...
export default function AccountantDashboard() {
    ...
    async function load() {
      try {
        const [cOrdRes, cDealRes, statsRes, pendingRes, ordersRes] = await Promise.all([
          ...
          fetch(`${BACKEND_URL}/orderhispegination?page=1&search=&id=0`)
            .catch(() => fetch(`${BACKEND_URL}/getMonthlyreporttoporder`)),
        ]);
        ...
        setRecentOrders(((ordersJson.data || ordersJson.top) || []).slice(0, 10));
      } catch (e) {
  ...
  const statCards = [
    ...
    { label: "Pending Payments",  value: `₹${pendingPayment.toLocaleString("en-IN")}`, icon: <AlertCircle size={15}/>,  bg: "bg-red-50",     text: "text-red-500",     border: "border-l-red-400"     },
  ];
